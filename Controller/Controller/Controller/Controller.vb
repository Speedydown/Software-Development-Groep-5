...
Public Class Controller
    ...
    Private Sub Controller(ByVal sender As Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles _controllerThread.DoWork
        Do While Not _controllerThread.CancellationPending
            ...
            SendMessage(numbers)
            _mainWindow.LogMessage("MESSAGE SENT: " + New Message().MessageToString(numbers))
        Loop
    End Sub

    Public Sub ReceiveMessage(message As Byte())
        '_message = New Message().DecodeMessage(message)

        'TODO Optimize.
        Dim messageAsString As String
        messageAsString = New Message().MessageToString(message)

        _mainWindow.LogMessage("MESSAGE RECEIVED: " + messageAsString)
    End Sub

    Private Sub SendMessage(ByVal parameters As Byte())

        _server.OutgoingMessage(parameters)
    End Sub
